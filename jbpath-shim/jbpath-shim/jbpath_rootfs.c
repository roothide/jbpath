...
extern int is_jbroot_name(const char* name);

struct dirent* JBPATH_SHIM_API(readdir)(DIR* dir)
{
...
int JBPATH_SHIM_API(readdir_r)(DIR* dir, struct dirent* d, struct dirent ** dp)
{
...
int JBPATH_SHIM_API(lstat)(const char * path, struct stat *sb)
{
...
int JBPATH_SHIM_API(lstatx_np)(const char *path, struct stat *sb, filesec_t fsec)
{
    const char* newpath = jbroot_alloc(path);
...
int JBPATH_SHIM_API(fstatat)(int fd, const char *path, struct stat *sb, int flag)
{
...
ssize_t JBPATHAT_SHIM_API(readlinkat)(int fd,const char* path,char* buf,size_t bufsize)
{
    const char* newpath = jbrootat_alloc(fd, path);
...
ssize_t JBPATH_SHIM_API(readlink)(const char * path, char* buf, size_t bufsiz)
{
    return JBPATH_SHIM_API(readlinkat)(AT_FDCWD, path, buf, bufsiz);
}
