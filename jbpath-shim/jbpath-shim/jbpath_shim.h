...
JBPATH_SHIM_WRAP(ssize_t, readlink, (const char * path, char* buf, size_t bufsiz), (newpath,buf,bufsiz), path)
...
//??int     unwhiteout(const char *); //not in dsc?
...
/* stdio.h */
JBPATH_SHIM_WRAP(FILE*, fopen, (const char * __restrict __filename, const char * __restrict __mode), (newpath,__mode), __filename)
...
JBPATH_SHIM_WRAP(int, lstat, (const char * path, struct stat *sb), (newpath,sb), path)
...
JBPATHAT_SHIM_WRAP(int, fstatat, (int fd, const char *path, struct stat *sb, int flag), (fd,newpath,sb,flag), fd, path, flag)
...
JBPATH_SHIM_WRAP(int, lstatx_np, (const char *path, struct stat *st, filesec_t fsec), (newpath, st, fsec), path)
...
JBPATHAT_SHIM_WRAP(ssize_t, readlinkat, (int fd,const char* path,char* buf,size_t bufsize), (fd,newpath,buf,bufsize), fd, path,0)
...
// !__DARWIN_ONLY_64_BIT_INO_T JBPATH_SHIM_WRAP(int, statfs64, (const char * path, struct statfs64 *buf), (newpath, buf), path)
