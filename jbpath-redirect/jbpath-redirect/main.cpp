...
int processShim(void* slice)
{
    ...
    for (int i = 0; i < header->ncmds; i++) {
        ...
        switch(lc->cmd) {
            ...
            case LC_ID_DYLIB: {
                struct dylib_command* idcmd = (struct dylib_command*)lc;
                g_shim_install_name = strdup((char*)((uint64_t)idcmd + idcmd->dylib.name.offset));
                LOG("install name = %s\n", g_shim_install_name);
                break;
            }
            ...
            case LC_DYLD_EXPORTS_TRIE: {
                export_trie = (struct linkedit_data_command*)lc;
                break;
            }
        }
...
//
    ...
    for(int i=0; i<symtab->nsyms; i++) {
        ...
        if(i>=dysymtab->iextdefsym && i<(dysymtab->iextdefsym+dysymtab->nextdefsym))
        {
            if(symstr[0]=='_' && strncmp(&symstr[1], STRIP_PREFIX, sizeof(STRIP_PREFIX)-1)==0)
            {
                int strl = strlen(symstr);
                memcpy(symstr, &symstr[1+sizeof(STRIP_PREFIX)-1], strl-(1+sizeof(STRIP_PREFIX)-1));
                memset(&symstr[strl-(1+sizeof(STRIP_PREFIX)-1)], 0, sizeof(STRIP_PREFIX)-1);
                
                LOG("new symstr=%s\n", symstr);
            }
            
            g_shim_apis.push_back(symstr);
        }
    }
    
    //temp imp: just remove trie instead of rebuild it
    if(export_trie) export_trie->cmd = 0;
    if(dyld_info) dyld_info->export_off = 0;
    if(dyld_info) dyld_info->export_size = 0;
    ...
    for (int i = 0; i < header->ncmds; i++) {
        ...
        switch(lc->cmd) {
            ...
            case LC_SEGMENT_64: {
                ...
                {
                    LOG("section[%d] = %s/%s offset=%x vm=%p:%p\n", j, sec[j].segname, sec[j].sectname,
...
int main(int argc, const char * argv[])
{
    ...
    if(processMachO(shimfile, processShim) < 0) {
        fprintf(stderr, "processShim error!\n");
        return -1;
    }
    ...
    if(processMachO(targetfile, processTarget) < 0) {
